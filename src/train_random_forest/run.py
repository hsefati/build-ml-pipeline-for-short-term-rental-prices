...
def go(args):
    ...
    logger.info("Exporting model locally")
    ...
    model_export_dir = "random_forest_dir"
    mlflow.sklearn.save_model(
        sk_model=sk_pipe,
        path=model_export_dir,
        serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
        signature=signature,
        input_example=X_val.iloc[:10],
    )

    logger.info("Uploading the the export model artifact")
    artifact = wandb.Artifact(
        args.output_artifact,
        type="model_export",
        description="Random Forest pipeline export",
    )
    artifact.add_dir(model_export_dir)
